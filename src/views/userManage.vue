		...
		<div class="outerbox-pro">
			<div class="fun-box">
				<div class="but-box">
					...
					<div class="but add-but" v-if="currentButtonJurisdiction.indexOf('add')>-1" @click="addFun"><i class="el-icon-circle-plus-outline"></i>新增账号</div>
				</div>
			</div>
					...
					<div class="listbox">
						<div class="list-outer-div">
							<div class="list-content-div">
								...
								<template>
									<el-table :data="listData" border :header-cell-style="{background:'#f2f2f2'}" style="width: 100%">
				...
				<div class="popup">
					...
					<div class="add-info-box">
						<div class="add-item-div">
							...
							<input class="add-item-input" type="text" v-model="currentItem.username" maxlength="20" />
						</div>
						<div class="add-item-div">
							<div class="add-item-title">所属单位</div>
							<input class="add-item-input" disabled="disabled" type="text" v-model="currentCompanyName" maxlength="20" />
						</div>
						<div class="add-item-div">
			...
			<el-dialog :visible.sync="dialogTableVisible_modifyPassword" :close-on-click-modal="false" width="25%">
				<div class="popup">
					...
					<div class="add-info-box">
						...
						<input class="add-item-input" type="password" v-model="modifyPassword.password" maxlength="50" />
					</div>
					<div class="popup-buts">
	...
	export default {
		...
		methods: {
			...
			addAndEditSubmit: function() {
				...
				let $this = this
				let loading = CommonFun.openFullScreen($this)
				if (CommonFun.ifNall($this.currentItem.username) || CommonFun.ifNall($this.currentItem.employeeId) || CommonFun.ifNall(
						$this.currentItem.roleId)) {
					CommonFun.closeFullScreen(loading)
					CommonFun.responseError({
						message: "姓名，员工，角色名都是必填！"
					}, $this);
					return;
				}
				axiosHttp
