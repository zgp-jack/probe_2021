...
<div class="m-dialog">
    <el-dialog :visible.sync="visible" :close-on-click-modal="false" width="846px">
		<div class="popup">
			...
			<div class="add-info-box" style="max-height: 600px">
				<el-form :model="queryData" :rules="rules" ref="formAddEdit" class="popupruleform" :disabled="staut == 0">
					<div class="alignBoth">
						...
						<el-form-item prop="mibId" label="厂家-型号" class="flex1 marginLeft10">
							<el-select v-model="queryData.mibId" class="add-item-input" clearable>
								<el-option v-for="item in relayMibList" :key="item.label" :label="item.label" :value="item.value"></el-option>
							</el-select>
							<!-- <el-input v-model="queryData.taskName" class="add-item-input" type="text" maxlength="64"></el-input> -->
						</el-form-item>
					</div>
					...
					<div class="alignBoth">
						...
						<el-form-item prop="cycle" label="采样周期" class="flex1 marginLeft10">
							<el-input v-model="queryData.cycle" class="add-item-input" type="number"></el-input>
						</el-form-item>
					</div>
					<!-- <div class="alignBoth">
						<el-form-item prop="specialLineFlag" label="是否绑定专线" class="flex1">
							<el-radio-group v-model="queryData.specialLineFlag">
								<el-radio :label="0">不绑定</el-radio>
								<el-radio :label="1">绑定</el-radio>
							</el-radio-group>
						</el-form-item>
						<el-form-item v-if="queryData.specialLineFlag == 1" prop="specialLineId" label="专线" class="flex1 marginLeft10">
							<el-input v-model="queryData.specialLineName" @click.native="dialogTableVisible_detail = true" class="add-item-input" type="text"></el-input>
						</el-form-item>
						<el-form-item prop="test" label="" class="flex1 marginLeft10">
						</el-form-item>
					</div> -->
...
export default {
    ...
    methods: {
		init() {
			...
			// this.queryData['manufactor'] = this.detailData['manufactor'];
			// this.queryData['model'] = this.detailData['model'];
			// this.queryData['specialLineFlag'] = this.detailData['specialLineFlag'];
		...
		selectChanged(value, type) {
			...
			ApiTaskRelay.listProbeDetail({deviceId: value}).then(function(res) {
		...
		submit() {
			// this.queryData.manufactor = this.relayMib[0];
			// this.queryData.model = this.relayMib[1];
			// this.queryData.mibId = this.relayMib;
			...
			this.$refs.formAddEdit.validate((valid) => {
				if (valid) {
					ApiTaskRelay.save(this.queryData).then(res => {
						const data = res.data;
						if (data.status === 1) {
