<template>
    <div class="content outerbox-pro">
        <div class="topruleform">
            ...
            <div class="but popup-but-submit gapright30" @click="handlerAdd">新增</div>
            <div class="but popup-but-submit gapright30" @click="sendTaskFun">下发任务</div>
            <div class="but popup-but-submit gapright30" @click="batchStop">批量暂停</div>
            <div class="but popup-but-submit" @click="batchDelete">批量删除</div>
        </div>
        <div class="listbox">
            <div class="list-outer-div">
                <div class="list-content-div">
                    <el-table 
                        ...
                        <el-table-column width="120" prop="probeIp" label="探针IP"></el-table-column>
                        <el-table-column width="120" :formatter="CommonFun.formatterCompanyName" label="组织机构" :show-overflow-tooltip="true"></el-table-column>
                        <el-table-column width="120" prop="probeInterfaceIp" label="拨测接口"></el-table-column>
                        ...
                        <el-table-column width="90" prop="cycle" label="采样周期" sortable></el-table-column>
                        <el-table-column width="70" prop="manufactor" label="厂家" :show-overflow-tooltip="true" :filters="filterManufactor" column-key="mibIdList"></el-table-column>
                        <el-table-column width="70" prop="model" label="型号" :show-overflow-tooltip="true"></el-table-column>
                        <el-table-column width="120" prop="gmtCreate" label="创建时间" sortable :show-overflow-tooltip="true"></el-table-column>
                        <el-table-column width="70" prop="statusName" label="任务状态"></el-table-column>
                        <!-- <el-table-column width="70" prop="specialLineFlag" label="绑定专线" :formatter="specialFormatter"></el-table-column>
                        <el-table-column width="70" prop="statusName" label="任务状态"></el-table-column> -->
                        <el-table-column width="110" label="操作" align="center">
                            <template slot-scope="scope">
                                ...
                                <div class="btnBox" title="节点对配置" @click="detailFun(scope.row)">
                                ...
                                <div class="btnBox" title="启用" v-if="scope.row.status==0 && currentButtonJurisdiction.indexOf('start')>-1"
                                    @click="statusFun(scope.row, 1)"><i class="el-icon-video-play"></i></div>
                                <div class="btnBox" title="停用" v-if="scope.row.status==1 && currentButtonJurisdiction.indexOf('stop')>-1"
        ...
        <span class="m-dialog">
            <el-dialog :visible.sync="dialogTableVisible_setting" v-if="dialogTableVisible_setting" :close-on-click-modal="false" width="690px">
                <div class="popup">
                    <div class="title">节点对管理</div>
                    <div class="hidepopup" @click="dialogTableVisible_setting=!dialogTableVisible_setting">×</div>
                    <div class="add-info-box">
                        <TaskRelayNodePair :targetIp="detailTaskId"/>
                    </div>
                    <div class="popup-buts">
                        <div class="popup-but popup-but-submit" @click="dialogTableVisible_setting= !dialogTableVisible_setting">确定</div>
                        <div class="popup-but popup-but-cancel" @click="dialogTableVisible_setting= !dialogTableVisible_setting">取消</div>
                    </div>
                </div>
            </el-dialog>
        </span>
...
export default {
    ...
    data() {
        return {
            ...
            currentButtonJurisdiction: CommonFun.getCurrentButtonJurisdiction('taskRelay'),
            listData: [],
            currentPage: 1,
			totle: 0,
            pageSize: 20,
    ...
    created() {
        this.getList().then((res, error) => {
            ...
            this.$store.dispatch('getRelayMib', {page: 1, pageSize: 999}).then(res => {})
        })
    },
    computed: {
		...mapState({
			probeDeviceList: state => state.probeDeviceList,
            relayMibList: state => state.relayMibList,
        ...
        filterManufactor() {
            return this.relayMibList.map(item => {
                return {text: item.companyName, value: item.value}
            })
        }
	},
    methods: {
        ...
        getList(page) {
            ...
            return ApiTaskRelay.list(param).then( res => {
                const data = res.data;
        ...
        detailFun: function(item) {
            this.detailTaskId = item.targetIp;
            this.dialogTableVisible_setting = true;
        },
        ...
        statusFun(item, status) {
            ...
            ApiTaskRelay.status({id: item.id, status: status}).then((res) => {
                CommonFun.closeFullScreen(loading)
        ...
        sendTaskFun() {
            ...
            ApiTaskRelay.send(params).then( res => {
                const data = res.data;
        ...
        batchStop(item) {
            ...
            let loading = CommonFun.openFullScreen(this)
            ApiTaskRelay.status(params).then((res) => {
                const data = res.data;
                CommonFun.closeFullScreen(loading);
                if (data.status === 1) {
                    this.$message({
        ...
        batchDeleteSubmit() {
            ...
            let params = {idList: list};
            let loading = CommonFun.openFullScreen(this);
            ApiTaskRelay.delete(params).then((res) => {
                const data = res.data;
                CommonFun.closeFullScreen(loading);
                if (data.status === 1) {
                    this.$message({
        ...
        submitDelete() {
            ApiTaskRelay.delete({id: this.detailData.id}).then( res => {
                const data = res.data;
