<template>
    <div class="content outerbox-pro">
        <div class="topruleform">
            ...
            <div class="gapright30 topruleform-inline">
                ...
                <el-select class="ellipsis-elselect" collapse-tags v-model="searchData.deviceIdList" filterable multiple placeholder="选择探针" clearable>
                    <el-option v-for="item in $store.state.probeDeviceList" :key="item.id" :label="item.name" :value="item.id"></el-option>
                </el-select>
            </div>
            <div class="gapright30 topruleform-inline">
                <label>组织机构：</label>
            ...
            <div class="gapright30 topruleform-inline">
                ...
                <el-select v-model="searchData.status" class="ellipsis-elselect" placeholder="任务状态" clearable>
                    <el-option v-for="item in CommonFun.getDataDictionaryChildrenListData($store.state.taskStatusValue)" :key="item.value"
            ...
            <div class="list-outer-div">
                <div class="list-content-div">
                    <el-table
                        ...
                        <el-table-column width="100" prop="targetIp" label="目的地址"></el-table-column>
                        <el-table-column width="90" prop="cycle" label="采样周期" sortable></el-table-column>
                        <el-table-column width="100" prop="manufactorName" label="厂家" :show-overflow-tooltip="true" :filters="filterManufactor" column-key="mibIdList"></el-table-column>
                        <el-table-column width="100" prop="model" label="型号" :show-overflow-tooltip="true"></el-table-column>
                        ...
                        <el-table-column width="120" label="操作" align="center">
                            <template slot-scope="scope">
                                <div class="btnBox" title="编辑" v-if="currentButtonJurisdiction.indexOf('edit') > -1" @click="handleEdit(scope.$index, scope.row)">
                                    <i class="el-icon-edit-outline"></i>
                                </div>
                                <div class="btnBox" title="配置接口" @click="detailFun(scope.row)">
                                    <i class="el-icon-link"></i>
                                </div>
                                <div class="btnBox" title="删除" v-if="currentButtonJurisdiction.indexOf('delete') > -1" @click="handleDelete(scope.$index, scope.row)">
                                    <i class="el-icon-delete"></i>
                                </div>
                                <div class="btnBox" title="启用" v-if="currentButtonJurisdiction.indexOf('start') > -1" v-show="scope.row.status == 0"
                                    @click="statusFun(scope.row, 1)">
                                    <i class="el-icon-video-play"></i>
                                </div>
                                <div class="btnBox" title="停用" v-if="currentButtonJurisdiction.indexOf('stop') > -1" v-show="scope.row.status == 1"
                                    @click="statusFun(scope.row, 0)">
        ...
        <SettingDialog
            v-if="dialogTableVisible_setting" 
            :visible.sync="dialogTableVisible_setting" 
            :title="title" 
            :staut="staut" 
            :detail-data="detailData"
            :search-data="searchDataDetail"
            @closeDialog="closeDialog" />
...
export default {
    name: 'taskDialTest',
	components: {
		SelectCompanyComponent, DetailDialog, SettingDialog
	},
    data() {
        return {
            ...
            currentButtonJurisdiction: CommonFun.getCurrentButtonJurisdiction('taskDevice'),
            listData: [],
            currentPage: 1,
			totle: 0,
            pageSize: 20,
    ...
    created() {
        this.getList().then((res, error) => {
            ...
            this.$store.dispatch('getDeviceMib', {page: 1, pageSize: 999}).then(res => {})
        })
    },
    computed: {
		...mapState({
            deviceMibList: state => CommonFun.getDataDictionaryChildrenListData(state.deviceManufactorValue),
		}),
        filterManufactor() {
            return this.deviceMibList.map(item => {
    ...
    methods: {
        ...
        getList(page) {
            ...
            return ApiTaskDevice.list(param).then( res => {
                const data = res.data;
        ...
        handlerAdd() {
            ...
            const list = CommonFun.getDataDictionaryChildrenListData(this.$store.state.taskValidityValue);
            for(let i = 0; i < list.length; i++) {
                validityCycleExArr.push(list[i].value);
            }
            this.detailData = {
        ...
        sendTaskFun() {
            ...
            ApiTaskDevice.status(params).then( res => {
                const data = res.data;
        ...
        batchDeleteSubmit() {
            ...
            let loading = CommonFun.openFullScreen(this);
            ApiTaskDevice.delete(params).then((res) => {
                const data = res.data;
                CommonFun.closeFullScreen(loading);
                if (data.status === 1) {
                    this.$message({
        ...
        statusFun(item, status) {
            let loading = CommonFun.openFullScreen(this)
            ApiTaskDevice.status({id: item.id, status: status}).then((res) => {
                CommonFun.closeFullScreen(loading)
        ...
        submitDelete() {
            ApiTaskDevice.delete({id: this.detailData.id}).then( res => {
                const data = res.data;
