    ...
    <div class="boxStyle">
		<div class="outerbox-pro">
            <analysisStatic ref="chartlist">
                ...
                <template #table>
                    <div class="interface-task"><span class="btn-check-chart" @click="changeTableFun">{{changeTable? '接口' : '任务'}}</span></div>
                    <div v-show="changeTable" class="listbox">
                            ...
                            <div class="list-content-div">
                                <template>
                                    <el-table v-loading="isLoading" :data="listData" border @sort-change='sortChangeFun' stripe :cell-style="{textAlign: 'center',height: '40px'}" :header-cell-style="{background:'rgba(10, 179, 172, .2)',height:'40px',textAlign:'center'}"  style="width: 100%">
                                        ...
                                        <el-table-column width="60"  prop="falg" label="标签" ></el-table-column>
                                        ...
                                        <el-table-column width="140" prop="duration" sortable='custom' :formatter="CommonFun.formatterContinuedTimeByKey" :show-overflow-tooltip="true" label="故障总时长"></el-table-column>
                                        ...
                                        <el-table-column width="140" prop="avgDuration" sortable='custom' :formatter="CommonFun.formatterContinuedTimeByKey" :show-overflow-tooltip="true" label="故障平均时长"></el-table-column>
                                        <el-table-column width="70" prop="healthRate" sortable='custom' :formatter="CommonFun.formatterParcentByKey" label="健康度"></el-table-column>
                                        <el-table-column width="70" prop="nowRate" sortable='custom' :formatter="CommonFun.formatterParcentByKey" label="在线率"></el-table-column>
                                        <el-table-column width="80" prop="statusName" label="当前状态"></el-table-column>
                                        <el-table-column width="60" label="详情" align="center">
                                            <template slot-scope="scope">
                            ...
                            <div class="list-content-div">
                                <template>
                                    <el-table v-loading="isLoading" :data="listData" border @sort-change='sortChangeFun' stripe :cell-style="{textAlign: 'center',height: '40px'}" :header-cell-style="{background:'rgba(10, 179, 172, .2)',height:'40px',textAlign:'center'}"  style="width: 100%">
                                        ...
                                        <el-table-column width="180" :formatter="CommonFun.formatterCompanyName" label="单位名称" :show-overflow-tooltip="true"></el-table-column>
                                        <el-table-column width="100"  prop="interfacesNumbers" sortable='custom' label="接口个数" ></el-table-column>
                                        ...
                                        <el-table-column width="70" label="详情" align="center">
                                            <template slot-scope="scope">
                                                <div class="btnBox" title="查看" @click="showFun(scope.$index, scope.row, 'hide')"><i class="el-icon-view"></i></div>
                                            </template>
                                        </el-table-column>
                                    </el-table>
                                </template>
...
export default {
    ...
    data() {
        return {
            searchData: {beginTime: '', endTime: '', taskType: 2},
            ...
            currentButtonJurisdiction: CommonFun.getCurrentButtonJurisdiction('analyseRelay')
        }
    },
    provide() {
        return {
            moduleName: 'analysisRelay',
    ...
    methods: {
        ...
        toPage(row) {
            if(row.count){
                ...
                this.$store.commit('pushIncludeList', 'analyseRelay');
                ...
                if(this.changeTable){
                    setTimeout(() => this.$router.push({name: toPage, params: {taskType: row.taskType, taskId: row.id, beginTime: this.Btime, endTime: this.Etime, anode: row.anode, bnode: row.bnode}}))
                }else{
        ...
        getList: function() {
            ...
            let analyseTaskUrl;
            if(this.changeTable){
                analyseTaskUrl = 'analyseTask/relayTaskStatisticsPageByInterface'
            }else {
                analyseTaskUrl = 'analyseTask/relayTaskStatisticsPageByTask'
            }
            $this.isLoading = true;
            return axiosHttp
                .post(baseUrl.BASEURL + analyseTaskUrl, params)
                .then(function(res) {
        ...
        sortChangeFun(column, prop, order){
            ...
            }else if(column.prop == 'interfacesNumbers'){
                $this.currentSort = 'interfaces_numbers'
            }else {
        ...
        exportAnalyseDegradation() {
            ...
            if(this.changeTable){
                exportUrl = 'analyseTask/exportRelayTaskStatisticsFileByInterface'
            }else {
                exportUrl = 'analyseTask/exportRelayTaskStatisticsFileByTask'
            }
            let loading = CommonFun.openFullScreen($this)
            axiosHttp
                .post(baseUrl.BASEURL + exportUrl, params)
                .then(function(res) {
        ...
        showFun(index,row,type) {
            ...
            let data = Object.assign(row,{
                ...
                taskType: row.taskType || 2,
                taskId: row.id,
                changeTable: this.changeTable
            })
            this.$store.commit('pushIncludeList', 'analyseRelay');
            sessionStorage.setItem('currentAramItem',JSON.stringify(data)) 
            setTimeout(() => $this.$router.push('/analyseRelaySpecialLineDetail'))
        },
