<template>
    <div class="p_chart">
        <p ref="deviceBar" style="height: 100%" v-show="companyData.companyPercent.length"></p>
        <div class="no-data-box"  v-show="!companyData.companyPercent.length">
            <img src="../../../assets/no-fault.png" />
            <p>暂无故障</p>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async init(data) {
            let that = this;
            that.chart = that.$echarts.init(that.$refs.deviceBar);
            ...
            that.chart.on('click', param => {
                that.toPage({companyId: param.data.companyId, companyName: param.data.companyName});
            })
        },
        resize() {
            this.$echarts.init(this.$refs.deviceBar).resize();
        }
    }
};
</script>
